<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 font-mono overflow-hidden relative">
    ...
    <main class="relative z-10 pt-20 sm:pt-24 px-4 sm:px-6">
      ...
      <section id="home" class="max-w-6xl mx-auto min-h-screen flex items-center py-12 sm:py-0">
        <div class="grid lg:grid-cols-2 gap-8 sm:gap-12 items-center w-full">
          <div class="space-y-6 sm:space-y-8">
            ...
            <!-- Stats HUD -->
            <div v-if="stats.length" class="hero-stats grid grid-cols-3 gap-2 sm:gap-4">
              <div
                v-for="stat in stats"
                :key="stat.label"
      ...
      <section v-if="companies.length" id="work" class="max-w-6xl mx-auto py-12 sm:py-16 md:py-24">
      ...
      <!-- Skills Section -->
      <section v-if="skills.length" id="skills" class="max-w-6xl mx-auto py-12 sm:py-16 md:py-24">
        <div class="section-title flex items-center gap-2 sm:gap-4 mb-8 sm:mb-12">
      ...
      <section v-if="projects.length" id="quests" class="max-w-6xl mx-auto py-12 sm:py-16 md:py-24">
        <div class="section-title flex items-center gap-2 sm:gap-4 mb-8 sm:mb-12">
          <div class="h-px flex-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent" />
          <h2 class="text-xl sm:text-2xl md:text-3xl font-bold text-center whitespace-nowrap">
            <span class="text-cyan-400">[</span>
      ...
      <!-- Credentials Section -->
      <section v-if="credentials.length" id="credentials" class="max-w-6xl mx-auto py-12 sm:py-16 md:py-24">
        <div class="section-title flex items-center gap-2 sm:gap-4 mb-8 sm:mb-12">
      ...
      <!-- Contact Section -->
      <section v-if="socialLinks.length" id="connect" class="max-w-6xl mx-auto py-12 sm:py-16 md:py-24 pb-44 sm:pb-32">
        <div class="section-title flex items-center gap-2 sm:gap-4 mb-8 sm:mb-12">
...
// Drop nav buttons for sections that have no data — mirrors the v-if guards
// on the actual <section> blocks below. HOME is always present (the hero
// renders unconditionally).
const navItems = computed(() => {
  const items = ['HOME']
  if (companies.value.length) items.push('WORK')
  if (skills.value.length) items.push('SKILLS')
  if (projects.value.length) items.push('QUESTS')
  if (credentials.value.length) items.push('CREDENTIALS')
  if (socialLinks.value.length) items.push('CONNECT')
  return items
})
...
// All landing content is now served from /api/v1/<collection>. The seed
// plugin in server/plugins/seed.ts populates these on first boot from the
// previously hardcoded values; the admin UI is the authoritative editor.
const { data: statsData } = await useFetch('/api/v1/stats')
const { data: companiesData } = await useFetch('/api/v1/companies')
const { data: skillsData } = await useFetch('/api/v1/skills')
const { data: projectsData } = await useFetch('/api/v1/projects')
const { data: socialData } = await useFetch('/api/v1/social')
const { data: credentialsData } = await useFetch('/api/v1/credentials')

const stats = computed(() => statsData.value?.items ?? [])
const companies = computed(() => companiesData.value?.items ?? [])
const skills = computed(() => skillsData.value?.items ?? [])
const socialLinks = computed(() => socialData.value?.items ?? [])
const credentials = computed(() => credentialsData.value?.items ?? [])
...
// `projects` is mutable — randomizeProjectIcons() in onMounted patches each
// item's icon/iconStyle in place. Keep it as a ref and mirror the API result
// (with empty icon fields ready for the randomizer).
const projects = ref([])
watch(projectsData, (val) => {
  if (!val?.items) return
  projects.value = val.items.map(p => ({ ...p, icon: null, iconStyle: {}, opacity: 1 }))
}, { immediate: true })
